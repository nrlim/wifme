...
import { Languages, Trash2, Plus, X, MessageCircle } from 'lucide-react';
...
function getLanguageColor(idx: number) {
...
export default function LanguageSettings({
  ...
  const defaultLanguages = ['Indonesia', 'Arab', 'Inggris'];
  const languages: string[] = initialSettings?.supportedLanguages?.length
    ? initialSettings.supportedLanguages
    : defaultLanguages;
  ...
  const paginatedLanguages = languages.slice((safePage - 1) * PAGE_SIZE, safePage * PAGE_SIZE);

  const handleDelete = async (lang: string) => {
    const ok = await confirm({
      title: 'Hapus Bahasa',
      message: `Yakin menghapus "${lang}"? Muthawif yang memilih bahasa ini mungkin terpengaruh.`,
    ...
    setIsSaving(true);
    try {
      await updateGlobalSettings({ supportedLanguages: languages.filter(l => l !== lang) });
      toast('success', 'Dihapus', `Bahasa ${lang} telah dihapus dari sistem.`);
    } catch (e: any) {
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    if (languages.some(l => l.toLowerCase() === val.toLowerCase())) {
      toast('error', 'Duplikasi', 'Bahasa tersebut sudah terdaftar.');
      return;
    }
    setIsSaving(true);
    try {
      await updateGlobalSettings({ supportedLanguages: [val, ...languages] });
      toast('success', 'Ditambahkan', `Bahasa ${val} berhasil didaftarkan.`);
      setNewLang('');
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
              ...
              <div>
                ...
                <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)', marginTop: 2 }}>
                  Tersedia sebagai pilihan komunikasi Muthawif
                </div>
              </div>
            </div>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 420 }}>
              ...
              <tbody>
                {paginatedLanguages.map((lang, idx) => {
                  ...
                  return (
                    <tr key={lang} className="ls-row" style={{ borderTop: '1px solid var(--border)' }}>
                  ...
                  <tr>
                    <td colSpan={4} style={{ padding: '4rem 1rem', textAlign: 'center' }}>
                      ...
                      <div style={{ fontSize: '0.8125rem', color: 'var(--text-muted)', marginTop: '0.25rem' }}>
                        Klik "Tambah Bahasa" untuk mendaftarkan bahasa operasi pertama.
                      </div>
                    </td>
                  </tr>
                )}
          ...
          <div style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>
                    Bahasa akan langsung aktif setelah disimpan
                  </div>
                </div>
              </div>
              <button
                type="button"
                onClick={() => { setShowAddModal(false); setNewLang(''); }}
                style={{
            ...
            <form onSubmit={handleAdd} style={{ padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
              ...
              {newLang.trim() && (
                <div style={{
                  ...
                  <span style={{
                    ...
                    <Languages size={12} /> {newLang.trim()}
                  </span>
                </div>
              )}
              ...
              <div style={{ display: 'flex', gap: '0.75rem', justifyContent: 'flex-end' }}>
                ...
                <button
                  type="submit"
                  disabled={isSaving || !newLang.trim()}
                  style={{
                    display: 'inline-flex', alignItems: 'center', gap: '0.5rem',
                    padding: '0.75rem 1.5rem', borderRadius: 12,
                    background: newLang.trim() ? 'linear-gradient(135deg, #1B6B4A, #27956A)' : '#E5E7EB',
                    color: newLang.trim() ? 'white' : '#9CA3AF',
                    border: 'none', fontWeight: 700, fontSize: '0.9375rem',
                    cursor: newLang.trim() ? 'pointer' : 'not-allowed',
                    fontFamily: 'inherit', transition: 'all 0.2s',
                    boxShadow: newLang.trim() ? '0 4px 14px rgba(27,107,74,0.3)' : 'none',
                  }}
                >
                  {isSaving ? <span className="spinner" style={{ width: 18, height: 18 }} /> : <Plus size={18} />}
                  {isSaving ? 'Menyimpan...' : 'Simpan Bahasa'}
                </button>
              </div>
            </form>
          </div>
