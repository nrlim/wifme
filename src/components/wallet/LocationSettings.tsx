        ...
        <div style={{
          ...
          <div className="ls-mobile-list" style={{ display: 'flex', flexDirection: 'column', gap: '0.85rem', padding: '1rem' }}>
            {paginatedLocations.map((loc, idx) => {
              ...
              const color = getLocationColor(globalIdx - 1);
              return (
                <div key={loc} style={{ background: 'white', borderRadius: 12, border: '1px solid var(--border)', padding: '1rem', display: 'flex', flexDirection: 'column', gap: '0.75rem', boxShadow: '0 1px 3px rgba(0,0,0,0.04)' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                      ...
                      <div>
                        <div style={{ fontWeight: 700, fontSize: '0.9375rem', color: 'var(--charcoal)' }}>{loc}</div>
                        <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>ID: {loc.toLowerCase().replace(/\s+/g, '-')}</div>
                      </div>
                    </div>
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '0.25rem' }}>
                    ...
                    <button type="button" onClick={() => handleDelete(loc)} disabled={isSaving} style={{ background: '#FEF2F2', color: '#EF4444', border: '1px solid #FECACA', borderRadius: 8, padding: '0.4rem 0.75rem', cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: '0.375rem', fontSize: '0.75rem', fontWeight: 600, fontFamily: 'inherit' }}>
            ...
            {paginatedLocations.length === 0 && (
              <div style={{ padding: '3rem 1rem', textAlign: 'center' }}>
                <div style={{ color: '#CBD5E1', marginBottom: '0.75rem' }}><Globe size={32} /></div>
                <div style={{ fontWeight: 700, color: 'var(--charcoal)', fontSize: '0.875rem' }}>Belum Ada Wilayah</div>
                <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>Klik Tambah Wilayah untuk mendaftar.</div>
              </div>
            )}
          </div>
