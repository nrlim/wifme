        ...
        >
          ...
          >
            ...
            <div style={{ flex: 1, overflowY: "auto" }}>
              ...
              {/* Extended Profile (Muthawif specific) */}
              {role === "MUTHAWIF" && muthawifProfile && (
                <div style={{ padding: "1.5rem", borderBottom: "1px solid var(--border)", background: "#FAFAFA" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1rem" }}>
                    <div style={{ fontSize: "0.8125rem", fontWeight: 800, color: "var(--charcoal)", textTransform: "uppercase", letterSpacing: "0.05em" }}>Profil Layanan</div>
                    <Link href="/dashboard/muthawif?tab=profile" onClick={() => setPanelOpen(false)} style={{ fontSize: "0.6875rem", fontWeight: 700, color: "var(--emerald)", textDecoration: "none" }}>Edit Detail</Link>
                  </div>
                  
                  <div style={{ display: "flex", flexDirection: "column", gap: "1.25rem" }}>
                    {/* Personal Branding (Bio) */}
                    <div>
                      <div style={{ fontSize: "0.6875rem", fontWeight: 700, color: "var(--text-muted)", marginBottom: "0.375rem" }}>Personal Branding (Bio)</div>
                      {muthawifProfile.bio ? (
                        <p style={{ fontSize: "0.8125rem", color: "var(--text-body)", lineHeight: 1.5, margin: 0, padding: "0.75rem", background: "white", borderRadius: "8px", border: "1px solid var(--border)" }}>
                          {muthawifProfile.bio}
                        </p>
                      ) : (
                        <div style={{ fontSize: "0.75rem", color: "var(--error)", background: "#FEF2F2", padding: "0.75rem", borderRadius: "8px", border: "1px dashed #FCA5A5" }}>
                          Bio belum diisi. Tambahkan bio untuk meyakinkan Jamaah!
                        </div>
                      )}
                    </div>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.75rem" }}>
                      <div style={{ background: "white", padding: "0.75rem", borderRadius: "8px", border: "1px solid var(--border)" }}>
                        <div style={{ fontSize: "0.6875rem", color: "var(--text-muted)", marginBottom: "0.25rem" }}>Tarif Dasar / Hari</div>
                        <div style={{ fontSize: "0.9375rem", fontWeight: 800, color: "var(--emerald)" }}>
                          Rp {(muthawifProfile.basePrice || 0).toLocaleString("id-ID")}
                        </div>
                      </div>
                      <div style={{ background: "white", padding: "0.75rem", borderRadius: "8px", border: "1px solid var(--border)" }}>
                        <div style={{ fontSize: "0.6875rem", color: "var(--text-muted)", marginBottom: "0.25rem" }}>Pengalaman</div>
                        <div style={{ fontSize: "0.9375rem", fontWeight: 800, color: "var(--brown)" }}>
                          {muthawifProfile.experience || 0} Tahun
                        </div>
                      </div>
                    </div>
                    ...
                    {/* Wilayah & Bahasa */}
                    <div>
                      <div style={{ fontSize: "0.6875rem", fontWeight: 700, color: "var(--text-muted)", marginBottom: "0.375rem" }}>Keahlian & Area</div>
                      <div style={{ display: "flex", flexWrap: "wrap", gap: "0.375rem" }}>
                        {(muthawifProfile.operatingAreas || []).map(area => (
                          <span key={area} style={{ fontSize: "0.6875rem", fontWeight: 600, color: "var(--emerald)", background: "var(--emerald-pale)", padding: "0.2rem 0.5rem", borderRadius: "6px" }}>{area}</span>
                        ))}
                        {(muthawifProfile.languages || []).map(lang => (
                          <span key={lang} style={{ fontSize: "0.6875rem", fontWeight: 600, color: "var(--charcoal)", background: "var(--ivory)", border: "1px solid var(--border)", padding: "0.2rem 0.5rem", borderRadius: "6px" }}>{lang}</span>
                        ))}
                      </div>
                    </div>
                  </div>
